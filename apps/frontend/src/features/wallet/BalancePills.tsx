...
interface BalancePillsProps {
  ...
  /** Renders a small brand-colored "+" button after the cash pill - the header's add-funds entry point. Omitted anywhere that action doesn't apply (the bet slip). */
  onAddFunds?: () => void;
}
    ...
    <span className={`flex items-center gap-1.5 ${className ?? ''}`}>
      ...
      {onAddFunds && (
        <button
          ...
          title="Add funds"
          className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand text-[11px] font-bold leading-none text-white transition-transform hover:brightness-110 active:scale-90"
        >
          +
        </button>
      )}
