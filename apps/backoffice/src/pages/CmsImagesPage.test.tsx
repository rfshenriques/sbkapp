...
import type { BrandImage, BrandImageListItem, CompetitionQuicklink } from '../lib/backendApi';
...
describe('CmsImagesPage', () => {
  ...
  it('switches to the Quicklinks tab, listing the ranked competition alongside every other one available to add', async () => {
    ...
    renderPage();
    await screen.findByText('Register - desktop');
    await userEvent.click(screen.getByRole('button', { name: 'Quicklinks' }));
  ...
  it('adding an available competition to Quicklinks posts it at the next order', async () => {
    const eplMatch: Match = {
      id: 'match-1',
      sport: 'Football',
      country: 'England',
